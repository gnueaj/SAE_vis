#!/usr/bin/env python3
"""
Calculate pairwise semantic distances between embedded explanations from three data sources.
"""
...
def calculate_all_pairwise_distances(
    embeddings_list: List[Dict],
    data_source_names: List[str],
    distance_metrics: List[str]
) -> Dict:
    """
    Calculate pairwise semantic distances between all combinations of three embedding sources.

    Args:
        embeddings_list: List of three embedding dictionaries
        data_source_names: List of three data source names
        distance_metrics: List of distance metrics to calculate
    ...
    for idx1, idx2, pair_name in pairs:
        print(f"\nCalculating distances for pair: {data_source_names[idx1]} vs {data_source_names[idx2]}")
...
def save_semantic_distances(
    ...
    """Save pairwise semantic distances data to JSON file and copy config."""
    ...
    print(f"Pairwise semantic distances saved to: {output_file}")
    print(f"Config saved to: {config_file}")


def main():
    """Main function to calculate pairwise semantic distances between three embedding sources."""
    parser = argparse.ArgumentParser(description="Calculate pairwise semantic distances between embeddings from three sources")
    parser.add_argument(
        "--config",
    ...
    output_dir = project_root / "data" / "semantic_distances" / f"{data_source_1}_vs_{data_source_2}_vs_{data_source_3}"
    ...
    print("\nCalculating pairwise semantic distances...")
    pairwise_results = calculate_all_pairwise_distances(
        embeddings_list,
        data_source_names,
    ...
    # Save results
    save_semantic_distances(final_data, str(output_dir), config["output_filename"], config, sae_ids, data_source_names)

    # Print summary statistics for each pair
    print("\n=== Summary Statistics ===")
    for pair_name, pair_data in pairwise_results.items():
        distances_data = pair_data["distances"]
        print(f"\n{pair_name}: {pair_data['data_source_1']} vs {pair_data['data_source_2']}")
        print(f"Total latents compared: {len(distances_data)}")
