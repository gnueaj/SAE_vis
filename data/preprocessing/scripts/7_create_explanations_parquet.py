#!/usr/bin/env python3
"""
Create Explanations Parquet

Extracts explanation text from detailed JSON files and creates a normalized
parquet file for efficient storage and querying of LLM-generated explanations.

Author: Data Processing Pipeline
Created: 2025-10-16
"""

import json
import argparse
from pathlib import Path
from datetime import datetime
from typing import Dict, List, Any
import polars as pl


def load_config(config_path: Path) -> Dict[str, Any]:
    """Load configuration from JSON file."""
    with open(config_path, 'r') as f:
        return json.load(f)


def find_project_root() -> Path:
    """Find the project root directory."""
    current = Path.cwd()
    while current != current.parent:
        if (current / 'data').exists() and (current / 'backend').exists():
            return current
        current = current.parent
    raise FileNotFoundError("Could not find project root (looking for data/ and backend/ directories)")


def process_feature_json(json_path: Path, sae_id: str) -> List[Dict[str, Any]]:
    """
    Extract explanation data from a single feature JSON file.

    Returns:
        List of dictionaries, one per explanation
    """
    with open(json_path, 'r') as f:
        data = json.load(f)

    feature_id = data['feature_id']
    explanations = data.get('explanations', [])

    rows = []

    for explanation in explanations:
        row = {
            'feature_id': feature_id,
            'sae_id': sae_id,
            'llm_explainer': explanation['llm_explainer'],
            'explanation_method': explanation['explanation_method'],
            'explanation_text': explanation['text']
        }
        rows.append(row)

    return rows


def create_explanations_parquet(config: Dict[str, Any], project_root: Path) -> Path:
    """
    Create explanations parquet from detailed JSON files.

    Returns:
        Path to created parquet file
    """
    # Resolve paths
    input_dir = project_root / config['input_directory']
    output_dir = project_root / config['output_directory']
    output_path = output_dir / config['output_filename']
    sae_id = config['sae_id']

    print(f"Input directory: {input_dir}")
    print(f"Output path: {output_path}")

    # Ensure output directory exists
    output_dir.mkdir(parents=True, exist_ok=True)

    # Process all JSON files
    all_rows = []
    json_files = sorted(input_dir.glob('feature_*.json'))

    print(f"\nProcessing {len(json_files)} feature files...")

    for i, json_path in enumerate(json_files, 1):
        if i % 100 == 0:
            print(f"  Processed {i}/{len(json_files)} files...")

        try:
            rows = process_feature_json(json_path, sae_id)
            all_rows.extend(rows)
        except Exception as e:
            print(f"  Warning: Failed to process {json_path.name}: {e}")
            continue

    print(f"Extracted {len(all_rows)} explanations")

    # Create DataFrame
    df = pl.DataFrame(all_rows)

    # Apply schema with proper types
    df = df.with_columns([
        pl.col('feature_id').cast(pl.UInt32),
        pl.col('sae_id').cast(pl.Categorical),
        pl.col('llm_explainer').cast(pl.Categorical),
        pl.col('explanation_method').cast(pl.Categorical),
        pl.col('explanation_text').cast(pl.Utf8)
    ])

    # Sort for consistent ordering
    df = df.sort(['feature_id', 'llm_explainer'])

    # Validate uniqueness
    unique_keys = df.select(['feature_id', 'llm_explainer']).unique()
    if len(unique_keys) != len(df):
        print(f"\n⚠ Warning: Duplicate (feature_id, llm_explainer) combinations detected!")
        print(f"  Total rows: {len(df)}, Unique combinations: {len(unique_keys)}")

    # Save parquet
    df.write_parquet(output_path)
    print(f"\n✓ Parquet file created: {output_path}")
    print(f"  Rows: {len(df)}")
    print(f"  Columns: {len(df.columns)}")
    print(f"  Unique features: {df['feature_id'].n_unique()}")
    print(f"  Unique explainers: {df['llm_explainer'].n_unique()}")

    return output_path


def create_metadata(config: Dict[str, Any], parquet_path: Path, project_root: Path) -> Path:
    """
    Create metadata JSON file for the parquet.

    Returns:
        Path to metadata file
    """
    # Load parquet to get statistics
    df = pl.read_parquet(parquet_path)

    # Calculate statistics
    unique_features = df['feature_id'].n_unique()
    unique_explainers = sorted(df['llm_explainer'].unique().to_list())
    unique_methods = sorted(df['explanation_method'].unique().to_list())

    # Calculate text length statistics
    text_lengths = df.select(pl.col('explanation_text').str.len_chars().alias('length'))
    text_stats = {
        'min': int(text_lengths['length'].min()),
        'max': int(text_lengths['length'].max()),
        'mean': float(text_lengths['length'].mean()),
        'median': float(text_lengths['length'].median())
    }

    # Count per explainer
    explainer_counts = {}
    for explainer in unique_explainers:
        count = len(df.filter(pl.col('llm_explainer') == explainer))
        explainer_counts[explainer] = count

    metadata = {
        'created_at': datetime.now().isoformat(),
        'parquet_file': str(parquet_path.relative_to(project_root)),
        'total_rows': len(df),
        'unique_features': unique_features,
        'unique_explainers': unique_explainers,
        'unique_methods': unique_methods,
        'explainer_counts': explainer_counts,
        'text_length_statistics': text_stats,
        'schema': {col: str(df[col].dtype) for col in df.columns},
        'config_used': config
    }

    # Save metadata
    metadata_path = parquet_path.with_suffix('.parquet.metadata.json')
    with open(metadata_path, 'w') as f:
        json.dump(metadata, f, indent=2)

    print(f"✓ Metadata file created: {metadata_path}")

    return metadata_path


def main():
    """Main execution function."""
    parser = argparse.ArgumentParser(
        description='Create explanations parquet from detailed JSON files'
    )
    parser.add_argument(
        '--config',
        type=Path,
        required=True,
        help='Path to configuration JSON file'
    )

    args = parser.parse_args()

    # Find project root
    try:
        project_root = find_project_root()
        print(f"Project root: {project_root}\n")
    except FileNotFoundError as e:
        print(f"Error: {e}")
        return 1

    # Load configuration
    config_path = args.config if args.config.is_absolute() else project_root / args.config
    print(f"Loading configuration from: {config_path}")
    config = load_config(config_path)

    # Create parquet
    parquet_path = create_explanations_parquet(config, project_root)

    # Create metadata
    create_metadata(config, parquet_path, project_root)

    print("\n✓ Explanations parquet creation complete!")
    return 0


if __name__ == '__main__':
    exit(main())
